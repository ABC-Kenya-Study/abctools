...
Functions that create derived variables are also included in `abctools`. Derived
variables are have the `calc_` prefix. Revisit this page to check for more
functions as they are added.
...
`abctools::` also has a straightforward workflow to prepare a flat, wide dataset
with one participant per row. Data cleaning functions and functions that create
derived variables are designed to be applied to the data collected from REDCap,
beforehand.
